...
var moreExpressParts = [];
var moreDescExpressParts = [];
$(".city-picker-dropdown .city-select-wrap a").on("click", function(){
    if($("#more-input-div").is(":visible")){
        $(".city-picker-dropdown .city-select-wrap dd .city-picker-dropdown").hide();
        var expressDesc = $(this).attr("title");
        var express = $(this).attr("data-code");
        addMore(expressDesc, express);
    }else {
        if($(this).next().is("div")){
        //if(isCtrlOn){
        //    alert($(this).attr("title") + ", " + $(this).attr("data-code") );
        //}
...
function addMore(expressDesc, express){
    var $search_container = $("#more-input-div .search-field");
    ...
    $close_link.on('click.chosen', (function(evt) {
        ...
        moreExpressParts = moreExpressParts.slice(0,expressIndex).concat(moreExpressParts.slice(expressIndex + 1));
        moreDescExpressParts = moreDescExpressParts.slice(0,expressIndex).concat(moreDescExpressParts.slice(expressIndex + 1));
        $(evt.target).parents('li').first().remove();
    }));
    $choice.append($close_link);
    $search_container.before($choice)
    moreExpressParts.push(express);
    moreDescExpressParts.push(expressDesc);
}
...
//        alert(expressDescParts.join(" ") + "\n" + expressParts.join(" "));
    ...
    add(expressDesc, express)

});

$("#more-input-div .search-field input").on("blur", function() {
    var value = $(this).val();
    if(value && value.trim()) {
        addMore(value, value)
    }
...
$(".more-rule-edit-ok").on("click", function () {

    var expressDesc = moreDescExpressParts.join(" ").trim();
    var express = moreExpressParts.join(" ").trim();
    add(express, expressDesc)
    moreDescExpressParts = [];
    moreExpressParts = [];
    $("#more-input-div ul li").not(".search-field").remove()
    $("#more-input-div").hide();

});

function add(express, expressDesc){
...
function refreshReview(){
    ...
    for(var index in expressDescArray) {
        if(index == 0) {
            ...
            var tmpExpressDesc = expressDescArray[index].join("<code><span style='color: red'> , </span></code>");
            var tmpExpress = expressArray[index].join("<code><span style='color: red'> , </span></code>");
...
$(".chosen-container.chosen-container-multi").live("click", function(){
    if(!$(this).is(".else-container") && !$(this).is(".chosen-container-more")) {
...
$("#submitBtn").on("click", function(){
    var id = $("#objectId").val();
    var value = $("#express").html().replace(/<code><span style="color: red"> &amp;&amp; <\/span><\/code>/g," && ")
                                        .replace(/<code><span style="color: red"> , <\/span><\/code>/g," , ")
...
$("#more-input-a").on("click", function(){
    $("#more-input-div").toggle();
});


var express = $("#express").html().replace(/<code><span style="color: red"> &amp;&amp; <\/span><\/code>/g," && ")
                                    .replace(/<code><span style="color: red"> , <\/span><\/code>/g," , ")
                                    .replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/&amp;/g,"&");
...
function getDescription(aExpress){
    ...
    if(!compareSymbolKey) {
        return varTitle + " " + unKnownExpress;
    }
