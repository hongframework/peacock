...
package org.apache.spark.sql.execution.smartsql.joins

import org.apache.spark.broadcast.Broadcast
import org.apache.spark.rdd.{UnionRDD, PartitionerAwareUnionRDD, ParallelCollectionRDD, RDD}
import org.apache.spark.sql.AnalysisException
import org.apache.spark.sql.catalyst.InternalRow
import org.apache.spark.sql.catalyst.expressions._
import org.apache.spark.sql.catalyst.plans._
import org.apache.spark.sql.catalyst.plans.physical._
import org.apache.spark.sql.execution.smartsql.{BinaryExecNode, SparkPlan}
import org.apache.spark.sql.execution.smartsql.metric.SQLMetrics
...
//    "numOutputRows" -> SQLMetrics.createMetric(sparkContext, "number of output rows"))
   ...
   */
  private def innerJoin(relation: Broadcast[Array[InternalRow]], parameters : Map[String, Any] = Map.empty[String, Any]): ResultSet[InternalRow] = {
    streamed.execute(parameters).mapPartitionsInternal { streamedIter =>
   ...
   */
  private def outerJoin(relation: Broadcast[Array[InternalRow]], parameters : Map[String, Any] = Map.empty[String, Any]): ResultSet[InternalRow] = {
    streamed.execute(parameters).mapPartitionsInternal { streamedIter =>
      val buildRows = relation.value
      val joinedRow = new JoinedRow
  ...
  private def leftExistenceJoin(
      ...
      exists: Boolean,
      parameters : Map[String, Any] = Map.empty[String, Any]): ResultSet[InternalRow] = {
  ...
  private def existenceJoin(relation: Broadcast[Array[InternalRow]], parameters : Map[String, Any] = Map.empty[String, Any]): ResultSet[InternalRow] = {
    assert(buildSide == BuildRight)
    streamed.execute(parameters).mapPartitionsInternal { streamedIter =>
      val buildRows = relation.value
      val joinedRow = new JoinedRow
   ...
   */
  private def defaultJoin(relation: Broadcast[Array[InternalRow]], parameters : Map[String, Any] = Map.empty[String, Any]): ResultSet[InternalRow] = {
    /** All rows that either match both-way, or rows from streamed joined with nulls. */
    val streamRdd = streamed.execute(parameters)
    ...
    joinType match {
      case LeftSemi =>
        ...
        return new ResultSet(buf.toIterator)//makeRDD(buf)
      case j: ExistenceJoin =>
        val buf: CompactBuffer[InternalRow] = new CompactBuffer()
        var i = 0
        val rel = relation.value
      ...
      case LeftAnti =>
        ...
        return new ResultSet(notMatched.toIterator)// makeRDD(notMatched)
      case o =>
    }
    ...
    ResultSet.union(
      matchedStreamRows,
      new ResultSet(notMatchedBroadcastRows.toIterator)
    )
  }

//  def makeRDD[T: ClassTag](seq: Seq[(T, Seq[String])]): RDD[T] =  {
//    val indexToPrefs = seq.zipWithIndex.map(t => (t._2, t._1._2)).toMap
//    new ParallelCollectionRDD[T](null, seq.map(_._1), scala.math.max(seq.size, 1), indexToPrefs)
//  }
