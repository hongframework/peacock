...
import org.apache.spark.bridge.CompressionCodec
...
import org.apache.spark.sql.execution.smartsql.metric.SQLMetric
import org.apache.spark.sql.types.DataType
import org.apache.spark.bridge.ThreadUtils
...
//    false
//  }
  val subexpressionEliminationEnabled = false
...
//    SparkSession.setActiveSession(sqlContext.sparkSession)
   ...
   */
  def longMetric(name: String): SQLMetric = new SQLMetric(name)
  ...
  /**
   * Returns the result of this query as an ResultSet[InternalRow] by delegating to `doExecute` after
   * preparations.
   *
   * Concrete implementations of SparkPlan should override `doExecute`.
   */
  final def execute(parameters : Map[String, Any] = Map.empty[String, Any]): ResultSet[InternalRow] = executeQuery {
...
//    }
   ...
   */
  protected def doExecute(parameters : Map[String, Any] = Map.empty[String, Any]): ResultSet[InternalRow]
   ...
   * compressed.
   */
  private def getByteArrayRdd(n: Int = -1, parameters : Map[String, Any] = Map.empty[String, Any]): ResultSet[Array[Byte]] = {
    execute(parameters).mapPartitionsInternal { iter =>
      var count = 0
   ...
   */
  def executeCollect(parameters : Map[String, Any] = Map.empty[String, Any]): Array[InternalRow] = {
    val byteArrayRdd = getByteArrayRdd(parameters = parameters)
  ...
  def executeTake(n: Int): Array[InternalRow] = {
    ...
    while (buf.size < n && partsScanned < totalParts) {
      ...
      if (partsScanned > 0) {
        ...
        val limitScaleUpFactor =  2// Math.max(sqlContext.conf.limitScaleUpFactor, 2)
        if (buf.isEmpty) {
      ...
      //todo
      val p = partsScanned.until(scala.math.min(partsScanned + numPartsToTry, totalParts).toInt)
