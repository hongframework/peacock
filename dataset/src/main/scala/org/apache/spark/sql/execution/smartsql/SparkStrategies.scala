...
import org.apache.spark.sql.execution.smartsql.columnar.InMemoryRelation
import org.apache.spark.sql.catalyst.InternalRow
import org.apache.spark.sql.catalyst.expressions._
import org.apache.spark.sql.catalyst.planning._
import org.apache.spark.sql.catalyst.plans._
import org.apache.spark.sql.catalyst.plans.logical._
import org.apache.spark.sql.execution.{ASortExec, Strategy, smartsql}
import org.apache.spark.sql.execution.smartsql.columnar.{InMemoryRelation, InMemoryTableScanExec}
import org.apache.spark.sql.execution.smartsql.joins.{BuildLeft, BuildRight}
...
abstract class SparkStrategies extends QueryPlanner[SparkPlan] {
   ...
   *     [[org.apache.spark.sql....broadcast()]] function to a DataFrame), then that side
...
//   * on-demand, only when planning in a [[org.apache.spark.sql.execution.smartsql.streaming.StreamExecution]]
//   */
...
//        LocalTableScanExec(output, sink.allData.map(r => encoder.toRow(r).copy())) :: Nil
      ...
      case logical.DeserializeToObject(deserializer, objAttr, child) =>
        DeserializeToObjectExec(deserializer, objAttr, planLater(child)) :: Nil
      case logical.SerializeFromObject(serializer, child) =>
        SerializeFromObjectExec(serializer, planLater(child)) :: Nil
      case logical.MapPartitions(f, objAttr, child) =>
        MapPartitionsExec(f, objAttr, planLater(child)) :: Nil
//      case logical.MapPartitionsInR(f, p, b, is, os, objAttr, child) =>
//        execution.MapPartitionsExec(
//          execution.r.MapPartitionsRWrapper(f, p, b, is, os), objAttr, planLater(child)) :: Nil
      case logical.FlatMapGroupsInR(f, p, b, is, os, key, value, grouping, data, objAttr, child) =>
        FlatMapGroupsInRExec(f, p, b, is, os, key, value, grouping,
          data, objAttr, planLater(child)) :: Nil
      case logical.MapElements(f, _, _, objAttr, child) =>
        MapElementsExec(f, objAttr, planLater(child)) :: Nil
      case logical.AppendColumns(f, _, _, in, out, child) =>
        AppendColumnsExec(f, in, out, planLater(child)) :: Nil
      case logical.AppendColumnsWithObject(f, childSer, newSer, child) =>
        AppendColumnsWithObjectExec(f, childSer, newSer, planLater(child)) :: Nil
      case logical.MapGroups(f, key, value, grouping, data, objAttr, child) =>
        MapGroupsExec(f, key, value, grouping, data, objAttr, planLater(child)) :: Nil
...
//        } else {
          CoalesceExec(numPartitions, planLater(child)) :: Nil
//        }
      case logical.Sort(sortExprs, global, child) =>
        ASortExec(sortExprs, global, planLater(child)) :: Nil
      case logical.Project(projectList, child) =>
        ProjectExec(projectList, planLater(child)) :: Nil
      case logical.Filter(condition, child) =>
        FilterExec(condition, planLater(child)) :: Nil
      case f: logical.TypedFilter =>
        FilterExec(f.typedCondition(f.deserializer), planLater(f.child)) :: Nil
...
//        execution.SampleExec(lb, ub, withReplacement, seed, planLater(child)) :: Nil
      ...
      case logical.LocalLimit(IntegerLiteral(limit), child) =>
        LocalLimitExec(limit, planLater(child)) :: Nil
      case logical.GlobalLimit(IntegerLiteral(limit), child) =>
        GlobalLimitExec(limit, planLater(child)) :: Nil
      case logical.Union(unionChildren) =>
        UnionExec(unionChildren.map(planLater)) :: Nil
