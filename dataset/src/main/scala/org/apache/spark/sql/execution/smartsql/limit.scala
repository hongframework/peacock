...
import org.apache.spark.sql.execution.smartsql.exchange.ShuffleExchange
...
case class CollectLimitExec(limit: Int, child: SparkPlan) extends UnaryExecNode {
  ...
  override def executeCollect(parameters : Map[String, Any] = Map.empty[String, Any]): Array[InternalRow] = child.executeTake(limit)
  private val serializer: Serializer = new UnsafeRowSerializer(child.output.size)
  protected override def doExecute(parameters : Map[String, Any] = Map.empty[String, Any]): ResultSet[InternalRow] = {
    val locallyLimited = child.execute(parameters).mapPartitionsInternal(_.take(limit))
...
trait BaseLimitExec extends UnaryExecNode with CodegenSupport {
  ...
  protected override def doExecute(parameters : Map[String, Any] = Map.empty[String, Any]): ResultSet[InternalRow] = child.execute(parameters).mapPartitions { iter =>
    iter.take(limit)
  }
...
case class TakeOrderedAndProjectExec(
  ...
  override def executeCollect(parameters : Map[String, Any] = Map.empty[String, Any]): Array[InternalRow] = {
    ...
    val data = child.execute(parameters).map(_.copy()).takeOrdered(limit)(ord)
  ...
  protected override def doExecute(parameters : Map[String, Any] = Map.empty[String, Any]): ResultSet[InternalRow] = {
    val ord = new LazilyGeneratedOrdering(sortOrder, child.output)
    val localTopK: ResultSet[InternalRow] = {
      child.execute(parameters).map(_.copy()).mapPartitions { iter =>
        org.apache.spark.util.collection.Utils.takeOrdered(iter, limit)(ord)
      }
    }
//    val shuffled = new ResultSet(
//      ShuffleExchange.prepareShuffleDependency(
