...
import com.hframework.smartsql.QueryPlanManager
 ...
 */
case class PlanSubqueries() extends Rule[SparkPlan] {
  def apply(plan: SparkPlan): SparkPlan = {
    plan.transformAllExpressions {
      case subquery: expressions.ScalarSubquery =>
        val executedPlan = new QueryPlanManager(subquery.plan).executedPlan
        ScalarSubquery(
