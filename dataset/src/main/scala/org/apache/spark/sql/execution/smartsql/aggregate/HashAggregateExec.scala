...
package org.apache.spark.sql.execution.smartsql.aggregate

import org.apache.spark.TaskContext
import org.apache.spark.memory.TaskMemoryManager
import org.apache.spark.sql.execution.{AUnsafeFixedWidthAggregationMap, AUnsafeKVExternalSorter}
import org.apache.spark.sql.execution.smartsql.ResultSet
import org.apache.spark.sql.execution.smartsql.{UnaryExecNode, SparkPlan}
...
import org.apache.spark.sql.execution.smartsql.metric.{SQLMetric, SQLMetrics}
...
//    "numOutputRows" -> SQLMetrics.createMetric(null, "number of output rows"),
//    "peakMemory" -> SQLMetrics.createSizeMetric(null, "peak memory"),
//    "spillSize" -> SQLMetrics.createSizeMetric(null, "spill size"),
//    "aggTime" -> SQLMetrics.createTimingMetric(null, "aggregate time"))
...
//    }
  ...
  protected override def doExecute(parameters : Map[String, Any] = Map.empty[String, Any]): ResultSet[InternalRow] = attachTree(this, "execute") {
    val numOutputRows = new SQLMetric("numOutputRows")
    val peakMemory = new SQLMetric("peakMemory")
    val spillSize = new SQLMetric("spillSize")


    child.execute(parameters).mapPartitions { iter =>
        ...
        val aggregationIterator =
          new TungstenAggregationIterator(
            ...
            (expressions, inputSchema) =>
              newMutableProjection(expressions, inputSchema, false),
            child.output,
            iter,
  ...
  override def inputRDDs(): Seq[ResultSet[InternalRow]] = {
   ...
   */
  def finishAggregate(
      hashMap: AUnsafeFixedWidthAggregationMap,
      sorter: AUnsafeKVExternalSorter,
...
//    }
    ...
    if (isFastHashMapEnabled) {
      if (isVectorizedHashMapEnabled) {
        ...
        ctx.addMutableState(
          "java.util.Iterator<org.apache.spark.sql.execution.smartsql.vectorized.ColumnarBatch.Row>",
    ...
    val hashMapClassName = classOf[AUnsafeFixedWidthAggregationMap].getName
    ctx.addMutableState(hashMapClassName, hashMapTerm, "")
    sorterTerm = ctx.freshName("sorter")
    ctx.addMutableState(classOf[AUnsafeKVExternalSorter].getName, sorterTerm, "")
    ...
    def outputFromVectorizedMap: String = {
        ...
        s"""
           ...
           |   $numOutput.add(1);
           |   org.apache.spark.sql.execution.smartsql.vectorized.ColumnarBatch.Row $row =
           |     (org.apache.spark.sql.execution.smartsql.vectorized.ColumnarBatch.Row)
  ...
  private def doConsumeWithKeys(ctx: CodegenContext, input: Seq[ExprCode]): String = {
    ...
    s"""
     ...
     ${
        if (isVectorizedHashMapEnabled) {
          s"""
             | org.apache.spark.sql.execution.smartsql.vectorized.ColumnarBatch.Row $fastRowBuffer = null;
           """.stripMargin
        } else {
          s"""
...
object HashAggregateExec {
  def supportsAggregate(aggregateBufferAttributes: Seq[Attribute]): Boolean = {
    ...
    AUnsafeFixedWidthAggregationMap.supportsAggregationBufferSchema(aggregationBufferSchema)
  }
}
