...
import org.apache.spark.sql.execution.smartsql.columnar.compression.{CompressibleColumnBuilder, AllCompressionSchemes}
...
private[columnar] class BasicColumnBuilder[JvmType](
  ...
  override def initialize(
    ...
    val size = if (initialSize == 0) ColumnBuilder.DEFAULT_INITIAL_BUFFER_SIZE else initialSize
  ...
  override def appendFrom(row: InternalRow, ordinal: Int): Unit = {
    buffer = ColumnBuilder.ensureFreeSpace(buffer, columnType.actualSize(row, ordinal))
