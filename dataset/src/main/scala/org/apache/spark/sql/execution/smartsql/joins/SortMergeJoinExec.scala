...
import org.apache.spark.sql.execution.smartsql.{BinaryExecNode, CodegenSupport,
 ...
 * 由于使用的是排序合并连接算法，该Exec内部并没有针对于左右的迭代器进行排序，
 * 因此要求外围的需要是进行排序过后的结果，因此我这里统一在CatalogRelationStrategy中进行处理
 */
...
//    "numOutputRows" -> SQLMetrics.createMetric(sparkContext, "number of output rows"))
  ...
  private def getSpillThreshold: Int = {128
//    sqlContext.conf.sortMergeJoinExecBufferSpillThreshold
  ...
  protected override def doExecute(parameters : Map[String, Any] = Map.empty[String, Any]): ResultSet[InternalRow] = {
    ...
    left.execute(parameters).zipPartitions(right.execute(parameters)) { (leftIter, rightIter) =>
...
//    do {
//      bufferedMatches.add(bufferedRow.asInstanceOf)
//      advancedBufferedToRowWithNullFreeJoinKey()
//    } while (bufferedRow != null && keyOrdering.compare(streamedRowKey, bufferedRowKey) == 0)
  }
}
