...
import org.apache.spark.sql.execution.smartsql.{ResultSet, BinaryExecNode, ExternalAppendOnlyUnsafeRowArray, SparkPlan}
...
//    "numOutputRows" -> SQLMetrics.createMetric(sparkContext, "number of output rows"))
  ...
  protected override def doExecute(parameters : Map[String, Any] = Map.empty[String, Any]): ResultSet[InternalRow] = {
    ...
    val leftResults = left.execute(parameters).asInstanceOf[RDD[UnsafeRow]]
    val rightResults = right.execute(parameters).asInstanceOf[RDD[UnsafeRow]]

    val spillThreshold = UnsafeExternalSorter.DEFAULT_NUM_ELEMENTS_FOR_SPILL_THRESHOLD.toInt//sqlContext.conf.cartesianProductExecBufferSpillThreshold
    ...
    }.asInstanceOf[ ResultSet[InternalRow]]
  }
}
