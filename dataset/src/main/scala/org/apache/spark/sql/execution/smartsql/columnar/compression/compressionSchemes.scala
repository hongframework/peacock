...
package org.apache.spark.sql.execution.smartsql.columnar.compression

import java.nio.ByteBuffer

import org.apache.spark.sql.execution.smartsql.columnar.compression
import org.apache.spark.sql.execution.smartsql.columnar
import org.apache.spark.sql.execution.smartsql.columnar._
...
private[columnar] case object PassThrough extends CompressionScheme {
  ...
  class Encoder[T <: AtomicType](columnType: NativeColumnType[T]) extends columnar.compression.Encoder[T] {
  ...
  class Decoder[T <: AtomicType](buffer: ByteBuffer, columnType: NativeColumnType[T])
    extends columnar.compression.Decoder[T] {
...
private[columnar] case object BooleanBitSet extends CompressionScheme {
  ...
  override def decoder[T <: AtomicType](buffer: ByteBuffer, columnType: NativeColumnType[T])
    : columnar.compression.Decoder[T] = {
    new this.Decoder(buffer).asInstanceOf[columnar.compression.Decoder[T]]
  }

  override def encoder[T <: AtomicType](columnType: NativeColumnType[T]): columnar.compression.Encoder[T] = {
    (new this.Encoder).asInstanceOf[columnar.compression.Encoder[T]]
  }

  override def supports(columnType: ColumnType[_]): Boolean = columnType == BOOLEAN

  class Encoder extends columnar.compression.Encoder[BooleanType.type] {
    private var _uncompressedSize = 0
  ...
  class Decoder(buffer: ByteBuffer) extends columnar.compression.Decoder[BooleanType.type] {
...
private[columnar] case object IntDelta extends CompressionScheme {
  ...
  override def decoder[T <: AtomicType](buffer: ByteBuffer, columnType: NativeColumnType[T])
    : columnar.compression.Decoder[T] = {
    new Decoder(buffer, INT).asInstanceOf[columnar.compression.Decoder[T]]
  }

  override def encoder[T <: AtomicType](columnType: NativeColumnType[T]): columnar.compression.Encoder[T] = {
    (new Encoder).asInstanceOf[columnar.compression.Encoder[T]]
  }

  override def supports(columnType: ColumnType[_]): Boolean = columnType == INT

  class Encoder extends columnar.compression.Encoder[IntegerType.type] {
  ...
  class Decoder(buffer: ByteBuffer, columnType: NativeColumnType[IntegerType.type])
    extends columnar.compression.Decoder[IntegerType.type] {
...
private[columnar] case object LongDelta extends CompressionScheme {
  ...
  override def decoder[T <: AtomicType](buffer: ByteBuffer, columnType: NativeColumnType[T])
    : columnar.compression.Decoder[T] = {
    new Decoder(buffer, LONG).asInstanceOf[columnar.compression.Decoder[T]]
  }
  ...
  class Encoder extends columnar.compression.Encoder[LongType.type] {
    protected var _compressedSize: Int = 0
    protected var _uncompressedSize: Int = 0
  ...
  class Decoder(buffer: ByteBuffer, columnType: NativeColumnType[LongType.type])
    extends columnar.compression.Decoder[LongType.type] {
