...
import org.apache.spark.sql.execution.smartsql.{ResultSet, UnaryExecNode, SparkPlan}
...
//  )
  ...
  protected override def doExecute(parameters : Map[String, Any] = Map.empty[String, Any]): ResultSet[InternalRow] = attachTree(this, "execute") {
    ...
    val fallbackCountThreshold = 128//sqlContext.conf.objectAggSortBasedFallbackThreshold

    child.execute(parameters).mapPartitionsInternal { iter =>
