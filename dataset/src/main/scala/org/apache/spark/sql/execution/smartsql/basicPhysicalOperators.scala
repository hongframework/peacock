...
case class ProjectExec(projectList: Seq[NamedExpression], child: SparkPlan)
  ...
  protected override def doExecute(parameters : Map[String, Any] = Map.empty[String, Any]): ResultSet[InternalRow] = {
    child.execute(parameters).mapPartitionsWithIndexInternal { (index, iter) =>
      val project = UnsafeProjection.create(projectList, child.output,
        false)
      project.initialize(index)
      val newIter = iter.map(project)
      newIter
    }
  }
...
////    "numOutputRows" -> SQLMetrics.createMetric(sparkContext, "number of output rows"))
//
//  protected override def doExecute(): ResultSet[InternalRow] = {
//    if (withReplacement) {
...
//      new EmptyResultSet[InternalRow](sqlContext.sparkContext)
//    } else {
...
case class UnionExec(children: Seq[SparkPlan]) extends SparkPlan {
  ...
  protected override def doExecute(parameters : Map[String, Any] = Map.empty[String, Any]): ResultSet[InternalRow] =
    ResultSet.union(children.map(_.execute(parameters)))
}
...
case class CoalesceExec(numPartitions: Int, child: SparkPlan) extends UnaryExecNode {
  ...
  protected override def doExecute(parameters : Map[String, Any] = Map.empty[String, Any]): ResultSet[InternalRow] = {
    child.execute(parameters).coalesce(numPartitions, shuffle = false)
  }
}
...
case class OutputFakerExec(output: Seq[Attribute], child: SparkPlan) extends SparkPlan {
  ...
  protected override def doExecute(parameters : Map[String, Any] = Map.empty[String, Any]): ResultSet[InternalRow] = child.execute(parameters)
}
